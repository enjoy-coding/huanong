...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 图片预览
            imageView: function (_option) {
                if ($.common.isEmpty(_option.className)) {
                    _option.className = '';
                }
                if ($.common.isEmpty(_option.style)) {
                    _option.style = '';
                }
                // blank or self
                if ($.common.isEmpty(_option.target)) {
                    _option.target = 'self';
                }
                if ($.common.isNotEmpty(_option.url)) {
                    return $.common.sprintf("<img class='img-circle img-xs %s' style='%s' data-target='%s' src='%s'/>", _option.className, _option.style, _option.target, _option.url);
                } else {
                    return $.common.nullToStr(null);
                }
            },
            // 搜索-默认第一个form
            ...
            importTemplate: function () {
            	if ($.table._option.importTemplate) {
            		window.location.href = $.table._option.importTemplate;
            	} else {
        ...
        treeTable: {
            ...
            init: function (options) {
                var defaults = {
                    id: options.eleId || "bootstrap-tree-table",
        ...
        operate: {
                ...
                var config = {
                    ...
                    success: function (result) {
                        if (typeof _option.callback == "function") {
                            _option.callback(result);
                        }
                        $.operate.ajaxSuccess(result, _option);
                    }
                };
                if (typeof _option.contentType === 'boolean') {
                    config.contentType = _option.contentType;
                }
                if (typeof _option.processData === 'boolean') {
                    config.processData = _option.processData;
                }
                $.ajax(config)
            },
            // post请求传输
            post: function (url, data, callback) {
                $.operate.submit({url: url, type: "post", dataType: "json", data: data, callback: callback});
            },
            // get请求传输
            get: function (url, callback) {
                $.operate.submit({url: url, type: "get", dataType: "json", data: "", callback: callback});
            },
            // 详细信息
            detail: function (id, width, height, ext) {
                var _url = $.operate.detailUrl(id, ext);
                var options = {
                    title: $.table._option.openTabTitle || ($.table._option.modalName + "详细"),
                    width: width,
                    height: height,
                    url: $.operate.detailUrl(id, ext),
            ...
            detailTab: function (id, ext) {
                $.modal.openTab($.table._option.openTabTitle || ($.table._option.modalName + "详细"), $.operate.detailUrl(id, ext));
            },
            // 详细访问地址
            detailUrl: function (id, ext) {
                var url = "/404.html";
            ...
            remove: function (id, ext) {
                $.modal.confirm("确定删除该条" + $.table._option.modalName + "信息吗？", function () {
                    ...
                    if ($.table._option.type == table_type.bootstrapTreeTable) {
                        ...
                        var data = {"ids": id};
                        $.operate.submit({url: url, type: "post", dataType: "json", data: data});
                    }
                });
            },
            // 批量删除信息
            ...
            clean: function () {
                $.modal.confirm("确定清空所有" + $.table._option.modalName + "吗？", function () {
                    ...
                    $.operate.submit({url: url, type: "post", dataType: "json", data: ""});
                });
            },
            // 添加信息
            add: function (id, ext) {
                $.modal.open($.table._option.openTabTitle || ("添加" + $.table._option.modalName), $.operate.addUrl(id, ext));
            },
            // 添加信息，以tab页展现
            addTab: function (id, ext) {
                $.modal.openTab($.table._option.openTabTitle || ("添加" + $.table._option.modalName), $.operate.addUrl(id, ext));
            },
            // 添加信息 全屏
            addFull: function (id, ext) {
                var url = $.table._option.createUrl;
                if ($.common.isNotEmpty(id)) {
                    url = url.replace("{id}", id);
                }
                url = url.replace("{ext}", ext || "");
                $.modal.openFull($.table._option.openTabTitle || ("添加" + $.table._option.modalName), url);
            },
            // 添加访问地址
            addUrl: function (id, ext) {
                var url = $.table._option.createUrl;
                if ($.common.isNotEmpty(id)) {
                    url = url.replace("{id}", id);
                }
            ...
            edit: function (id, ext) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    url = url.replace("{ext}", ext || "");
                    $.modal.open($.table._option.openTabTitle || ("修改" + $.table._option.modalName), url);
                } else {
                    $.modal.open($.table._option.openTabTitle || ("修改" + $.table._option.modalName), $.operate.editUrl(id, ext));
                }
            },
            // 修改信息，以tab页展现
            editTab: function (id, ext) {
                $.modal.openTab($.table._option.openTabTitle || ("修改" + $.table._option.modalName), $.operate.editUrl(id, ext));
            },
            // 修改信息 全屏
            editFull: function (id, ext) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                ...
                url = url.replace("{ext}", ext || "");
                $.modal.openFull($.table._option.openTabTitle || ("修改" + $.table._option.modalName), url);
            },
            // 修改访问地址
            editUrl: function (id, ext) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.updateUrl.replace("{id}", id);
                } else {
            ...
            save: function (_option) {
                var config = {
                    ...
                    success: function (result) {
                        ...
                        $.operate.successCallback(result, _option);
                    }
                };
                if (typeof _option.contentType === 'boolean') {
                    config.contentType = _option.contentType;
            ...
            saveTab: function (_option) {
                ...
                var config = {
                    ...
                    success: function (result) {
                        ...
                        $.operate.successTabCallback(result, _option);
                    }
                };
                if (typeof _option.contentType === 'boolean') {
                    config.contentType = _option.contentType;
             ...
             * @param opt 操作
             */
            otherOpt: function (id, ext, opt, optName) {
                //为树形表格菜单
                if ($.table._option.type == table_type.bootstrapTreeTable) {
                    //这段逻辑暂时没有用到，不考虑@@@@@@2
                    if ($.common.isEmpty(id)) {
                        ...
                        if ($.table._option.otherOpts[opt]) {
                            var optUrl = $.table._option.otherOpts[opt];
                            var url = optUrl.replace("{id}", row[$.table._option.uniqueId]);
                            url = url.replace("{ext}", ext || "");
                            $.modal.open(optName + $.table._option.modalName, url);
                        } else {
                            $.modal.alertWarning("该操作地址不存在，请联系管理员!");
                        }
                    } else {
                        if ($.table._option.otherOpts[opt]) {
                            var optUrl = $.table._option.otherOpts[opt];
                            var url = optUrl.replace("{id}", id);
                            url = url.replace("{ext}", ext || "");
                            $.modal.open(optName + $.table._option.modalName, url);
                        } else {
                            $.modal.alertWarning("该操作地址不存在，请联系管理员!");
                    ...
                    //非树形表格菜单的情况，大多数情况下是这个菜单，所以优先考虑这个里面逻辑
                    //操作存在，就进入
                    if ($.table._option.otherOpts[opt]) {
                        $.modal.open(optName + $.table._option.modalName, $.operate.otherOptUrl(id, ext, opt));
                    } else {
                        $.modal.alertWarning("该操作地址不存在，请联系管理员!");
                    }
                }
            },
            otherOptUrl: function (id, ext, opt) {
                var url = "/404.html";
                var optUrl = $.table._option.otherOpts[opt];
                if ($.common.isNotEmpty(id)) {
                    url = optUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
            ...
            // 保存结果弹出msg刷新table表格
            ajaxSuccess: function (result, _option) {// _option暂时无用
            ...
            // 成功回调执行事件（父窗体静默更新）
            successCallback: function (result, _option) {
                if (result.code == web_status.SUCCESS) {
                    var parent = window.parent;
                    var successFunc = function () {
                        if (_option && typeof _option.success == "function") {
                            _option.success(result, _option);
                        } else if (_option && _option.success == "close") {
                            $.modal.close();
            ...
            // 选项卡成功回调执行事件（父窗体静默更新）
            successTabCallback: function (result, _option) {
                if (result.code == web_status.SUCCESS) {
                    var topWindow = $(window.parent.document);
                    var currentId = $('.page-tabs-content', topWindow).find('.active').attr('data-panel');
                    var $contentWindow = $('.Feather_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
