...
$(function() {
    ...
    function setActiveTab(element) {
        if (!$(element).hasClass('active')) {
            ...
            // 显示tab对应的内容区
            $('.Feather_iframe').each(function() {
                if ($(this).data('id') == currentId) {
                	var sHeight = this.contentDocument.documentElement.scrollTop || this.contentDocument.body.scrollTop;
                  	$(element).attr("scroll-y", sHeight);
                    $(this).show().siblings('.Feather_iframe').hide();
                    return false;
                }
            });
    ...
    function scrollToTab(element) {
        ...
        var sHeight = $(element).attr("scroll-y");
        if (sHeight) {
        	var dataId = $(element).attr("data-id");
            $('.mainContent .Feather_iframe').each(function() {
                if ($(this).data('id') == dataId) {
                    this.contentWindow.scrollTo(0, sHeight);
                    return false;
                }
            });
        }
    ...
    function menuItem() {
        ...
        if (flag) {
            ...
            $('.mainContent .Feather_iframe').each(function() {
                if ($(this).data('id') == dataId) {
                	var sHeight = this.contentDocument.documentElement.scrollTop || this.contentDocument.body.scrollTop;
                    $(".menuTab.active").attr("scroll-y", sHeight);
                    return false;
                }
            });
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="Feather_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            $('.mainContent').find('iframe.Feather_iframe').hide().parents('.mainContent').append(str1);
    ...
    function menuBlank() {
    	// 新窗口打开外网以http://开头，如http://feather.vip
    	var dataUrl = $(this).attr('href');
    	window.open(dataUrl);
    	return false;
    }
    ...
    function activeTab() {
        if (!$(this).hasClass('active')) {
        	var element = $('.menuTab.active');
        	var actvieId = element.data('id');
        	$('.mainContent .Feather_iframe').each(function() {
                if ($(this).data('id') == actvieId) {
                	var sHeight = this.contentDocument.documentElement.scrollTop || this.contentDocument.body.scrollTop;
                	element.attr("scroll-y", sHeight);
                	return false;
                }
    ...
    function refreshTab() {
    	...
    	var target = $('.Feather_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.Feather_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            $('.Feather_iframe[data-id="' + $(this).data('id') + '"]').show();
    ...
    $.contextMenu({
        ...
        items: {
            ...
            "full": {
                ...
                callback: function(key, opt) {
                	setActiveTab(this);
                    var target = $('.Feather_iframe[data-id="' + this.data('id') + '"]');
