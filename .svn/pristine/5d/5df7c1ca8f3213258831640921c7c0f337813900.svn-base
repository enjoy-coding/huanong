package com.feather.quartz.service.impl;
...
import com.feather.quartz.mapper.SysJobMapper;
import com.feather.quartz.service.ISysJobService;
import com.feather.quartz.util.CronUtils;
import com.feather.quartz.util.ScheduleUtils;
...
public class SysJobServiceImpl implements ISysJobService {
    ...
    public int pauseJob(SysJob job) throws SchedulerException {
        ...
        job.setStatus(Constants.FAIL_OR_DISABLED);
    ...
    public int resumeJob(SysJob job) throws SchedulerException {
        Long jobId = job.getJobId();
        String jobGroup = job.getJobGroup();
        job.setStatus(Constants.SUCCESS_OR_ENABLED);
        int rows = jobMapper.updateJob(job);
        if (rows > 0) {
    ...
    public int changeStatus(SysJob job) throws SchedulerException {
        ...
        if (Constants.SUCCESS_OR_ENABLED.equals(status)) {
            rows = resumeJob(job);
        } else if (Constants.FAIL_OR_DISABLED.equals(status)) {
            rows = pauseJob(job);
        }
        return rows;
    }
    ...
    public int insertJob(SysJob job) throws SchedulerException, TaskException {
        ...
        if (StringUtils.isBlank(job.getConcurrent())) {
            job.setConcurrent(Constants.FAIL_OR_DISABLED);
        }
